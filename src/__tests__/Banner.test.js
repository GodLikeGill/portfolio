...
test('it shows the title in the banner', () => {
	setup();
	// We expect that the title 'Hi, Iam Jatin' is in the banner component
	expect(screen.getByText(/Hi, Iam Jatin/i)).toBeInTheDocument();
});
...
test('can download cv when clicked on download cv button', async () => {
	...
	// expect(downloadLink.download).toEqual('Jatin-Resume.pdf');
	// expect(downloadLink.href).toEqual('/files/Jatin-Resume.pdf');
